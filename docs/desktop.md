...
- Starts persisted background scans through `start_scan_job_for_root`.
- Polls buffered scan job events through `scan_job_events`.
- Queries the SQLite index through `query_index`.
- Searches indexed files through `search_files`.
- Loads duplicate group file previews through `duplicate_group_files`.
